...
// Based on https://github.com/ybbus/jsonrpc
...
func (client *rpcClient) CallWID(method string, params ...interface{}) (*RPCResponse, error) {
...
func (client *rpcClient) doCall(RPCRequest *RPCRequest) (*RPCResponse, error) {
	...
	err = jsoniter.Unmarshal([]byte(s), &rpcResponse)
...
func (RPCResponse *RPCResponse) GetInt() (int64, error) {
	val, ok := RPCResponse.Result.(jsoniter.Number)
	if !ok {
...
func (RPCResponse *RPCResponse) GetObject(toType interface{}) error {
	...
	if RPCResponse.Result == "OK" {
		RPCResponse.Result = RPCResponse.Data
	}

	js, err := jsoniter.Marshal(RPCResponse.Result)
	if err != nil {
		return err
	}

	err = jsoniter.Unmarshal(js, toType)
	if err != nil {
		return err
	}
